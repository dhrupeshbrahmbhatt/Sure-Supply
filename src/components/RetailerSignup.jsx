...
import { FaUser, FaEnvelope, FaPhone, FaArrowRight, FaEye, FaEyeSlash } from 'react-icons/fa';
...
const RetailerSignup = () => {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3000/retailer/send-otp', {
        method: 'POST',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:3000/retailer/signup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
              ...
              <motion.div variants={itemVariants}>
                <div className="relative">
                  ...
                  <input
                    ...
                    onBlur={() => setFocusedField(null)}
                    className="w-full pl-12 pr-12 py-4 bg-white/50 rounded-2xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all duration-300"
                    required
                  />
                  <button
                    type="button"
              ...
              <motion.div variants={itemVariants}>
                ...
                >
                  <span className="flex items-center justify-center gap-2">
                    {!otpSent ? 'Verify Number' : 'Create Business Account'}
                    <FaArrowRight />
                  </span>
                </button>
              </motion.div>
